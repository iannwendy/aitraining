"""Train BERTopic-only and PhoBERT + BERTopic on augmented dataset (offline mode).
Uses local cached models without network access.

Usage:
  .venv/bin/python scripts/train_bertopic_models_offline.py
"""
...
SENTENCE_TRANSFORMER_PATH = Path.home() / ".cache/huggingface/hub/models--sentence-transformers--paraphrase-multilingual-MiniLM-L12-v2/snapshots/e8f8c211226b894fcb81acc59f3b34ba3efd5f42"
...
def load_sentence_transformer():
    ...
    return SentenceTransformer(str(SENTENCE_TRANSFORMER_PATH))
...
class BERTopicOnlyClassifier:
    ...
    def __init__(self, topic_model_path):
        ...
        logger.info("  Loading SentenceTransformer (local)...")
        self.embedder = load_sentence_transformer()
    ...
    def _get_topic_features(self, texts):
        ...
        logger.info(f"    Encoding {len(texts_list)} texts...")
        embeddings = self.embedder.encode(texts_list, show_progress_bar=False, batch_size=64)
...
def train_bertopic_only(train_texts, train_labels, test_texts, test_labels, cross_texts, cross_labels):
    ...
    logger.info("  Predicting on cross-domain set...")
...
class PhoBertTopicClassifier:
    ...
    def __init__(self, phobert_model_dir, topic_model_path):
        ...
        logger.info(f"  Loading PhoBERT from {phobert_model_dir}...")
        ...
        logger.info("  Loading SentenceTransformer (local)...")
        self.topic_embedder = load_sentence_transformer()
        ...
        self.max_len = PHOBERT_MAX_LENGTH
    ...
    def _get_phobert_embeddings(self, texts):
        ...
        texts_list = list(texts)
        logger.info(f"    Extracting PhoBERT embeddings for {len(texts_list)} texts...")

        prepared = prepare_many_texts(texts_list)
        dataset = PhoBertDataset(prepared, None, self.tokenizer, self.max_len)
    ...
    def _get_topic_features(self, texts):
        texts_list = list(texts)
        logger.info(f"    Encoding for BERTopic: {len(texts_list)} texts...")
...
def train_phobert_bertopic(train_texts, train_labels, test_texts, test_labels, cross_texts, cross_labels):
    ...
    model = PhoBertTopicClassifier(PHOBERT_PATH, MODEL_DIR / "bertopic" / "bertopic_model.pkl")
    model.fit(train_texts, train_labels)
