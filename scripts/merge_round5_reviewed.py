...
def recover_gold_sets(raw_gold: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    if len(base) != 2072:
        raise ValueError(f"Expected 2,072 unique pre-R5 training-gold rows, recovered {len(base):,}")
    if len(r5) != 1360:
        raise ValueError(f"Expected 1,360 unique Round-5 rows, recovered {len(r5):,}")
