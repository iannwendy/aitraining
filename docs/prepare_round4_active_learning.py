"""Active Learning Round 4 — Select samples for human review and create Label Studio import files.

This script selects samples for the 4th round of blind human annotation.
Selection strategy prioritizes samples with highest information value:
  1. Weak-label vs PhoBERT disagreement (most valuable)
  2. Boundary probability (0.45–0.55)
  3. Low confidence (0.55–0.70)
  4. PhoBERT high confidence but weak-label uncertain

Target: Add ~500-1000 new gold samples, especially depression-positive ones.

Outputs (in docs/):
  - label_studio_round4_active_learning_import.csv  (BLIND — only row_id + text)
  - label_studio_round4_active_learning_key.csv     (full columns for merge later)

Usage:
  .venv/bin/python docs/prepare_round4_active_learning.py
"""
...
ROUND3_REVIEW_FILE = DATA_DIR / "round3_review.csv"

# ── Outputs ──────────────────────────────────────────────────────────
IMPORT_FILE = DOCS_DIR / "label_studio_round4_active_learning_import.csv"
KEY_FILE = DOCS_DIR / "label_studio_round4_active_learning_key.csv"
REPORT_FILE = DOCS_DIR / "round4_selection_report.json"

# ── Settings ─────────────────────────────────────────────────────────
TOTAL_SAMPLES = 1500  # Target samples for this round
DISAGREEMENT_TARGET = 500    # Weak-label vs PhoBERT disagree
BOUNDARY_TARGET = 400       # Probability 0.45-0.55
LOW_CONFIDENCE_TARGET = 300  # Probability 0.55-0.70
HIGH_CONF_UNCERTAIN_TARGET = 300  # PhoBERT confident but weak uncertain
RANDOM_SEED = 42
...
def main() -> None:
    ...
    # Also exclude round3 review texts
    round3 = pd.read_csv(ROUND3_REVIEW_FILE, dtype=str).fillna("")
    round3_texts = set(round3["comment_text"].str.strip().tolist())

    already_reviewed = gold_texts | al_texts | round3_texts
    print(f"Already reviewed: {len(already_reviewed):,} unique texts "
          f"(gold={len(gold_texts):,}, active_learning={len(al_texts):,}, round3={len(round3_texts):,})")
    ...
    # 4d. High confidence PhoBERT but weak-label uncertain
    high_conf_uncertain_mask = (
        weak_uncertain
        & ~disagreement_mask
        & ~boundary_mask
        & ~low_conf_mask
        & (preds["probability"] >= 0.70)
    )
    high_conf_uncertain = preds[high_conf_uncertain_mask].copy()
    high_conf_uncertain["review_bucket"] = "high_conf_uncertain"

    print(f"\nBucket sizes before sampling:")
    print(f"  Disagreement:           {len(disagreement):,}")
    print(f"  Boundary probability:    {len(boundary):,}")
    print(f"  Low confidence:         {len(low_conf):,}")
    print(f"  High conf, weak unc.:   {len(high_conf_uncertain):,}")
    ...
    buckets = [
        ...
        ("high_conf_uncertain", high_conf_uncertain, HIGH_CONF_UNCERTAIN_TARGET),
    ]
    ...
    combined["row_id"] = [f"r4-{i:04d}" for i in range(len(combined))]
    ...
    # Count depression vs normal from PhoBERT
    phobert_depression = sum(1 for l in combined["phobert_label"] if l == "depression")
    phobert_normal = sum(1 for l in combined["phobert_label"] if l == "normal")
    ...
    report = {
        ...
        "already_reviewed_round3": int(len(round3_texts)),
        "remaining_pool_size": int(len(preds)),
        "bucket_stats": bucket_stats,
        "phobert_label_distribution": phobert_label_counts,
        "phobert_depression_count": phobert_depression,
        "phobert_normal_count": phobert_normal,
        ...
        "instructions": (
            "1. Import label_studio_round4_active_learning_import.csv into Label Studio "
            "(Settings → Labeling Interface → Code — use the same XML config as before). "
            "2. After review, export CSV and rename to export_round4_active_learning.csv. "
    ...
    print(f"\n{'='*60}")
    print(f"ACTIVE LEARNING ROUND 4 — READY")
    print(f"{'='*60}")
    print(f"Selected: {len(combined):,} samples")
    print(f"PhoBERT label distribution: {phobert_label_counts}")
    print(f"  - Depression: {phobert_depression}")
    print(f"  - Normal: {phobert_normal}")
    ...
    print(f"   2. Review samples")
    print(f"   3. Export as export_round4_active_learning.csv")
