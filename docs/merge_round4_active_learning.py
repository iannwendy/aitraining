"""Merge Round 4 Active Learning review results into gold + final_dataset.

Usage:
  .venv/bin/python docs/merge_round4_active_learning.py

Prerequisites:
  1. Run round4 selection: docs/prepare_round4_active_learning.py
  2. Import into Label Studio and review
  3. Export CSV → docs/export_round4_active_learning.csv
"""
...
# ── Inputs ───────────────────────────────────────────────────────────
EXPORT_FILE = DOCS_DIR / "export_round4_active_learning.csv"
...
REPORT_FILE = DOCS_DIR / "round4_merge_report.json"
...
def main() -> None:
    ...
    # Find the label column in export (could be 'depression', 'final_label', etc.)
    label_col = None
    possible_labels = ["depression", "final_label", "label", "annotations"]
    ...
    # Update final_label with exported label
    ...
    # Separate valid from uncertain/exclude
    ...
    # New entries become human_gold with weight=3
    ...
    # Combine with existing final_dataset
    ...
    # Split new combined dataset into train/val/test
    ...
    print(f"\n{'='*60}")
    print(f"ROUND 4 MERGE COMPLETE")
    print(f"{'='*60}")
